...
public class HeaderExchangeServer implements ExchangeServer {
    ...
    private void startIdleCheckTask(URL url) {
        if (!server.canHandleIdle()) { // 底层NettyServer自己有心跳机制，则上层的ExchangeServer就不用开启心跳任务了
            AbstractTimerTask.ChannelProvider cp = () -> unmodifiableCollection(HeaderExchangeServer.this.getChannels());
            int idleTimeout = getIdleTimeout(url);
            long idleTimeoutTick = calculateLeastDuration(idleTimeout);
            CloseTimerTask closeTimerTask = new CloseTimerTask(cp, idleTimeoutTick, idleTimeout); // 定义关闭Channel的Task
            this.closeTimerTask = closeTimerTask;
            // init task and start timer.
            IDLE_CHECK_TIMER.newTimeout(closeTimerTask, idleTimeoutTick, TimeUnit.MILLISECONDS); // 定时运行closeTimerTask
        }
    }
}
